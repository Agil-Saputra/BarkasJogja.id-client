...
export default function Home() {
	const {response : categoriesResponse} = useAxios('categories?populate=*')
	const {response : newestProducts} = useAxios('products?populate=*&sort=createdAt:desc')
	const {response : locations} = useAxios('locations?populate=*')
  ...
  return (
    <>
      ...
      <div className="flex items-center mt-4 mb-2 justify-between">
        ...
        <a href="/produk" className="text-primary underline">
          Lihat Semua
        </a>
      </div>

      <div className="grid grid-cols-2 gap-3 justify-between" id='newest-products'>
		{newestProducts[0] ? (newestProducts.slice(0, 8).map(({attributes} : ProductsDatasProps) => (
			<ProductCard slug={attributes.slug} ambilDitempat={attributes.ambilDitempat} jarakMaksimal={attributes.jarakMaksimal} nego={attributes.nego} judul={attributes.title} harga={attributes.harga} kecamatan={attributes.kecamatan} images={attributes.product_images} kabupaten={attributes.kabupaten.data?.attributes.kabupaten} productImageURl={attributes.product_images.data[0].attributes.url} postDate={attributes.createdAt}/>
      ...
      <div className="grid max-[380px]:grid-cols-2 grid-cols-3  justify-between gap-4">
        {locations[0] ? (locations.map((location : LocationsProps) => (
			<LocationCard key={location.id} title={location.attributes.kabupaten} logoURL={location.attributes.locationLogo.data.attributes.url}/>
